  ...
  return (
    <form onSubmit={handleSubmit}>
      <h1>SendNFT</h1>
        ...
        {data ?  <Stack space="5" direction={"horizontal"} justify="space-between" wrap>{data?.assets?.map((collectible) => {
          const key = collectible.contractAddress + '-tokenId-' + collectible.tokenId
         
          const checked = selected.includes(key)
          
          return <RadioNFT key={key} image={collectible?.imageUrl} label={collectible?.collectionName?.slice(0, 20)} address={collectible?.contractAddress}  checked={checked} tokenId={collectible?.tokenId} onChange={(e) => {
