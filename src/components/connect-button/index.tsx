  ...
  const renderUserProfile = () => {
    return (
      <Box className={styles.userProfile}>
        {ensAvatar ? <Avatar size="6" label={`${user?.alias} profile`} src={ensAvatar} /> :
          <Avatar size="6" label={`${user?.alias} profile`} address={user?.blockchainAccounts[0]?.address} placeholder />}
        <Box fontWeight={"semiBold"}>{user?.alias}</Box>
      </Box>
    );
  };
